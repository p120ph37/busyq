...
/* --- GNU coreutils (per-command entry points for LTO pruning) ---
 *
 * Each command maps directly to its single_binary_main_TOOL() entry
 * point from coreutils' --enable-single-binary build.  This bypasses
 * the argv[0]-based dispatcher, giving LTO a static call graph so it
 * can prune unused commands in custom builds.
 *
 * Special names (from coreutils' gen-single-binary.sh):
...
/* GNU gawk — awk is an alias dispatching via argv[0] */
...
/* GNU grep — egrep/fgrep dispatch via argv[0] */
...
/* GNU diffutils — each command has its own entry point */
...
/* GNU findutils — find and xargs have separate entry points */
...
/* GNU gzip — gunzip/zcat dispatch via argv[0] */
...
/* bzip2 — bunzip2/bzcat dispatch via argv[0] */
...
/* xz-utils — multiple aliases dispatch via argv[0] */
...
/* GNU bc — bc and dc have separate entry points */
...
/* dos2unix — unix2dos dispatches via argv[0] */
...
/* GNU sharutils — uuencode/uudecode have separate entry points */
...
/* ncurses tset/reset — reset dispatches via argv[0] */
...
/* procps-ng — each tool has its own entry point */
...
/* pkill dispatches via argv[0] — shares pgrep entry */
...
/* psmisc — each tool has its own entry point */
